...
const imageElement = document.getElementById("cover") as HTMLImageElement
...
fetch(`https://open.fixspotify.com/api/info/${type}/${id}`).then(res => res.json()).then(async (data) => {
    switch (type) {
        case "track": {
            const track = data
            imageElement.src = track.albumArt
            titleElement.innerText = track.name
            descriptionElement.innerText = `By ${track.artists} • ${track.album}`
            break
        }
        case "album": {
            const album = data
            imageElement.src = album.albumArt ? album.albumArt : placeholder
